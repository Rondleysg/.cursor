...
5. **Create or reuse Page Objects:** Para browser, `pageobjects/<dominio>/` com seletores **WebdriverIO** apenas: [WebdriverIO Selectors](https://webdriver.io/docs/selectors), [skill-pr-selectors-for-automation](../skill-pr-selectors-for-automation/SKILL.md). **`data-testid` / `testID`:** padrão **`<feature>-<component>-<element>`** (ex.: `login-form-submit`). **XPath proibido**; sem Playwright. Ver [skill-qa-webdriverio-maintain-conventions](../skill-qa-webdriverio-maintain-conventions/SKILL.md). Export singleton; métodos públicos + getters para asserts.
