...
4. **Write specs:** Import `expect` from `@wdio/globals`; use `getDeviceFromCapabilities('browser')` or `getDeviceFromCapabilities('mobile')` from `lib/Utils`; use Page Objects (browser) or Screen Objects (app); Arrange-Act-Assert; for data, import from test-data or use the builder. Use **Allure**: structure (addEpic, addFeature, addStory), steps; when useful add severity, tag, attachment, issue/testId, argument (see [docs/10-allure-reporter.md](../../../docs/10-allure-reporter.md) — "API Allure – quando usar").
