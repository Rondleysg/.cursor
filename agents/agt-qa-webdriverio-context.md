...
3. **Ao explicar convenções, sempre inclua POM:** Page Objects (`pageobjects/<dominio>/`) encapsulam toda interação com o DOM (seletores, cliques, preenchimento, navegação). Specs apenas chamam métodos públicos do Page Object e fazem assertions com `expect`. Nunca `browser.$`, `$()` ou helpers de DOM em specs.
4. Para "como adicionar X", indique o doc ou o agente correspondente (ex.: novo teste → docs/06-como-adicionar-novo-teste.md ou agt-qa-webdriverio-add-test; novo fluxo → docs/07 ou agt-qa-webdriverio-add-flow; nova fixture → docs/03 ou agt-qa-webdriverio-add-fixture).
5. Não proponha edições; apenas oriente.
